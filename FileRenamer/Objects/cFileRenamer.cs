...
namespace FileRenamer
{
    public class cFileRenamer
    {
        ...
        #region Methods
            ...
            private static void ProcessFoldername(string input_path, cSettings settings)
            {
                try
                {
                    string input_directory_name         = input_path.Substring(input_path.LastIndexOf('\\') + 1);
                    string output_directory_name        = input_directory_name;
                    string current_path                 = input_path.Substring(0,input_path.LastIndexOf('\\') + 1);
                    ...
                    if (output_directory_name != input_directory_name && !string.IsNullOrEmpty(output_directory_name))
                    {
                        ...
                        if (Directory.Exists(output_directory_name) && (output_directory_name.ToLower() != input_path.ToLower()))
                        {
                            settings.ErrorList.Add("Cannot rename " + input_path + " -> " + output_directory_name + ", directory already exists.");
                            return;
                        }

                        FileIo.RenameDirectory(input_path, output_directory_name);
                        settings.ChangeList.Add(input_path + " -> " + output_directory_name);
                    }
            ...
            private static void ProcessFilename(string input_path, cSettings settings)
            {
                if (string.IsNullOrWhiteSpace(input_path))
                    throw new Exception("File path is null or empty");

                if (!File.Exists(input_path))
                    throw new Exception(string.Format("File '{0}' does not exist", input_path));

                if (settings == null)
                    throw new Exception("Settings object is null");

                string input_filename = Path.GetFileName(input_path);
                string output_path = Path.GetDirectoryName(input_path);
                string output_filename = input_filename;

                try
                {
                    // check to see if we need to reset file properties
                    if (settings.FixFileProperties && File.GetAttributes(input_path) != FileAttributes.Normal)
                        File.SetAttributes(input_path, FileAttributes.Normal);
                    ...
                    if (input_filename != output_filename && !string.IsNullOrEmpty(output_filename))
                    {
                        ...
                        {
                            settings.ErrorList.Add("Cannot rename " + input_path + " -> " + output_path + ", file already exists.");
                            return;
                        }

                        FileIo.RenameFile(input_path, output_path);
                        settings.ChangeList.Add(input_path + " -> " + output_path);
                    }
                }
                catch (Exception ex)
                {
                    settings.ErrorList.Add(string.Format("Cannot rename {0} -> {1}, exception: {2}", input_path, output_path, ex.Message));
                }
            }
            ...
            public static void RunScript(string script_path, ref cSettings settings)
            {
                var script_instructions = FileIo.ReadLinesFromFile(script_path);
